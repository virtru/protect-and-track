...
if test -f "$FILE"; then
  ...
  # slack integration
  docker run --rm -it -v ${PWD}/reports:/virtru/reports virtru/automated-test:latest /bin/bash -c "PYTHONPATH=. python3 tools/send_slack_notification.py"

  # pagerduty integration
  docker run --rm -it -v ${PWD}/reports:/virtru/reports virtru/automated-test:latest /bin/bash -c "PYTHONPATH=. python3 tools/send_pagerduty_notification.py"
fi
